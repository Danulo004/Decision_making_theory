    ...
    for i in range(len(matrix)):
        for j in range(len(matrix)):
            ...
            if result != 0 and j not in deletedRows:
                ...
                print("Стретегія А" + str(i + 1), "домінуюча над стратегією А" + str(j + 1), "тому забираємо рядок",
                      j + 1)
    ...
    for i in range(len(transposedMatrix)):
        for j in range(len(transposedMatrix)):
            ...
            if result != 0 and j not in deletedColumns:
                ...
                print("Стретегія В" + str(i + 1), "домінуюча над стратегією В" + str(j + 1), "тому забираємо стовпчик",
                      j + 1)
...
print(Fore.WHITE + "Визначимо максимальне значення цільової функції", Fore.RED, mainCondition + '--> max',
      Fore.WHITE + "при настуних умовах-обмеженнях:", Style.RESET_ALL)
print(conditions)
print(Fore.WHITE + "Після переведення в канонічну форму переходимо до основно алгоритму симплекс-методом",
      Style.RESET_ALL)
