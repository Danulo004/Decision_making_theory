...
def draw_parameters(parameters, objects, maxWidths):
    ...
    for parameter in parameters:
        ...
        coefficientPart = Fore.LIGHTRED_EX + ('%' + str(maxWidths['COEFFICIENT']) + 's') % parameters[parameter][
            'COEFFICIENT'] + Style.RESET_ALL
        objectsPart = ''
...
def print_total(header, objects, parameters, maxWidths):
    ...
    print(totalPart)
    print(header_line)
    print('Best object has ' + Fore.RED + '(' + maxValue + ')' + Style.RESET_ALL)
...
for param in parameters_equations:
    for obj in objects:
        parameters_equations[param][obj] = str(parameters_equations[param]['COEFFICIENT']) + '*' + str(
            parameters_equations[param][obj])
